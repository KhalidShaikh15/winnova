...
export default function LeaderboardPage() {
    ...
    const [selectedSlot, setSelectedSlot] = useState<string>(''); // Default to empty
    ...
    useEffect(() => {
        ...
        const fetchLeaderboard = async () => {
            ...
            // 1. Fetch all match results for the tournament
            const matchesQuery = query(
                collection(firestore, 'matches'),
                where('tournament_id', '==', selectedTournament)
            );
            const matchesSnapshot = await getDocs(matchesQuery);
            const allMatchesList = matchesSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as MatchResult));
            ...
            allMatchesList.forEach(match => {
                ...
                const matchesPlayedForTeam = new Set(allMatchesList.filter(m => m.registration_id === teamId).map(m => m.match_number)).size;
    ...
    return (
        <div className="container py-12">
            ...
            {loadingLeaderboard ? (
                ...
                <Card><CardContent className="p-6 text-center">No leaderboard data found for this tournament yet. Results will appear here after matches are scored.</CardContent></Card>
            )}
        </div>
    );
}
